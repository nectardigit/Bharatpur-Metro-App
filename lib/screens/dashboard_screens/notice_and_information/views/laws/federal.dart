...
class LawFederalLevel extends StatelessWidget {
  LawFederalLevel({Key? key});
  final controller = Get.put(AppController());
  @override
  Widget build(BuildContext context) {
    List<LawData> lawfederalFilter =
        controller.lawlist.where((e) => e.lawLevel == '4').toList();
    // var list = controller.lawtypeList;
    return Scaffold(
      appBar: buildAppbar(
          controller.isNepali.value ? ' संघीय कानुन' : ' Federal Laws'),
      body: SingleChildScrollView(
          child: Column(children: [
        ListView.builder(
            shrinkWrap: true,
            itemCount: lawfederalFilter.length,
            physics: const BouncingScrollPhysics(),
            padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 10),
            itemBuilder: (context, index) {
              var e = lawfederalFilter[index];
              return CustomTile(
                ontap: () {
