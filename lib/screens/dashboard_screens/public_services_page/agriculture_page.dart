...
class AgriculturePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<PublicSericeHeadingModel> agricultureService = controller.publicService
        .where((e) => e.serviceCategoryId! == 5)
        .toList();
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          agricultureService.isNotEmpty
              ? ListView.builder(
                  shrinkWrap: true,
                  itemCount: agricultureService.length,
                  itemBuilder: (context, index) {
                    var data = agricultureService[index];
                    return CustomTile(
                      ontap: () {
